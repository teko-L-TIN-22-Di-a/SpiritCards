...
class ActionHandler:
    ...
    def resolve_summon(self, instance: ActionInstance):
        ...
        target_slot: Slot = next(requirement.value for requirement in instance.requirements if requirement.requirement.key == Requirement.TARGET_FREE_SLOT)
        cost: int = next(requirement.value for requirement in instance.requirements if requirement.requirement.key == Requirement.MANA_COST)
