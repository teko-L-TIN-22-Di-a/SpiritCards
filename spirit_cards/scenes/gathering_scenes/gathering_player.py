...
class GatheringPlayer(IsometricEntity):
    ...
    def __init__(self, context: Context):
        ...
        self._temp_movement = pygame.Vector3(0,0,0)
    ...
    def update(self, delta: float) -> None:
        ...
        self._temp_movement = temp_movement

    def check_colliders(self, position: pygame.Vector3):
        for collider in self.colliders:
            if self.precise_collision(collider, position.x, position.z):
                return True
        return False

    def precise_collision(self, collider: pygame.Rect, x: int, z:int):
        precise_collider = pygame.Rect(
            collider.left*100,
            collider.top*100,
            collider.width*100,
            collider.height*100
        )
        return precise_collider.collidepoint(x*100,z*100)
